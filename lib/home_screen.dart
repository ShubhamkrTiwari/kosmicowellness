...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final String playstoreUrl = updateInfo['playStoreUrl']?.toString() ?? updateInfo['playstore_url']?.toString() ?? updateInfo['url']?.toString() ?? updateInfo['link']?.toString() ?? 'https://play.google.com/store/apps/details?id=com.kosmico.wellness';
              final Uri targetUri = Uri.parse(playstoreUrl);
              try {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          TextButton(
            onPressed: () async {
              final String playstoreUrl = _updateData?['playStoreUrl']?.toString() ?? _updateData?['playstore_url']?.toString() ?? _updateData?['url']?.toString() ?? _updateData?['link']?.toString() ?? 'https://play.google.com/store/apps/details?id=com.kosmico.wellness';
              final Uri targetUri = Uri.parse(playstoreUrl);
              try {
                await launchUrl(targetUri, mode: LaunchMode.externalApplication);
              } catch (e) {
  ...
  bool _checkAuthAndProceed(VoidCallback onSuccess) {
  ...
  Widget _buildProductCard(Map<String, dynamic> product, ColorScheme colorScheme, LanguageManager lang) {
    ...
    return GestureDetector(
      onTap: () => _checkAuthAndProceed(() {
        Navigator.of(context).push(MaterialPageRoute(builder: (context) => ProductDetailsScreen(product: product))).then((_) { if (mounted) setState(() {}); });
      }),
      child: Card(
        elevation: 0,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              Text(name, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: colorScheme.onSurface), maxLines: 1, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 2),
