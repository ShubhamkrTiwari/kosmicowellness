...
class ApiService {
  ...
  static const String baseUrl = 'https://api.kosmicowellness.com/api';

  // Helper to ensure clean URLs without double slashes and avoid /api/api duplication
  ...
  static Future<Map<String, dynamic>> createCodUpfrontOrder({
    required double amount,
    ...
    try {
      final url = _getUri('/api/payment/cod-upfront/create');
      ...
      debugPrint('API: Creating COD Upfront Order at $url');
  ...
  static Future<Map<String, dynamic>> verifyCodUpfrontPayment({
    ...
    try {
      final url = _getUri('/api/payment/cod-upfront/verify');
      final body = jsonEncode({
      ...
      debugPrint('API: Verifying COD Upfront Payment at $url');
      final response = await http.post(
        url,
        headers: {
          'Content-Type': 'application/json',
  ...
  static Future<Map<String, dynamic>> getComments(String postId, String token) async {
    try {
      final response = await http.get(
      ...
      // Fallback: try fetching single post which might contain populated comments
      ...
      return _processResponse(postRes);
    } catch (e) {
      return _handleError(e);
    }
  }
  ...
  static Future<Map<String, dynamic>> addComment(String postId, String text, String token) async {
    try {
      ...
      final currentId = UserManager().userId;
      if (currentId != null && currentId.isNotEmpty) {
        UserCache.set(currentId, resolvedName, UserManager().profilePicture);
      }
  ...
  static Future<void> checkForAppUpdate() async {
    try {
      final response = await http.get(
        _getUri('/api/updates/check?version=1.0.3'),
        headers: _getHeaders(),
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data is Map) {
          await _handleAppVersionInfo({
            'latest_version': data['latest_version'] ?? data['version'] ?? '1.0.3',
            'min_required_version': data['min_required_version'] ?? data['minVersion'] ?? '1.0.3',
            'force_update': data['update_required'] == true || data['force_update'] == true,
            'playStoreUrl': data['playStoreUrl'] ?? data['playstore_url'],
          });
        }
      }
    } catch (e) {
      debugPrint('Check for app update error: $e');
    }
  }
      ...
      if (hasBody) {
        try {
          ...
          if (data is Map) {
            ...
            if (data['app_version_info'] != null) {
              _handleAppVersionInfo(Map<String, dynamic>.from(data['app_version_info']));
            }
            if (data['latest_version'] != null || data['update_required'] == true) {
              _handleAppVersionInfo(Map<String, dynamic>.from(data));
            }
          }
        } catch (_) {}
      }
  ...
  static bool _isVersionLower(String currentVersion, String targetVersion) {
    try {
      List<int> currentParts = currentVersion.split('.').map((e) => int.tryParse(e.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0).toList();
      List<int> targetParts = targetVersion.split('.').map((e) => int.tryParse(e.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0).toList();
  ...
  static Future<void> _handleAppVersionInfo(Map<String, dynamic> info) async {
    // Check if admin has toggled update prompt OFF via ON/OFF toggle
    bool isUpdateEnabled = info['update_enabled'] ?? 
                           info['isUpdateEnabled'] ?? 
                           info['enabled'] ?? 
                           info['show_update'] ?? 
                           info['showUpdate'] ?? 
                           true;
    ...
    if (isUpdateEnabled == false) {
      return; // Admin turned OFF the update prompt
    }

    final String currentVersion = '1.0.3';
    final String latestVersion = info['latest_version']?.toString() ?? info['version']?.toString() ?? currentVersion;
    final String minRequiredVersion = info['min_required_version']?.toString() ?? info['minVersion']?.toString() ?? currentVersion;
    final bool forceUpdateFlag = info['force_update'] == true || info['update_required'] == true;
    final String playstoreUrl = info['playStoreUrl']?.toString() ?? info['playstore_url']?.toString() ?? 'https://play.google.com/store/apps/details?id=com.kosmico.wellness';

    bool isBelowMin = _isVersionLower(currentVersion, minRequiredVersion);
    bool isBelowLatest = _isVersionLower(currentVersion, latestVersion);

    if (forceUpdateFlag || isBelowMin) {
      _showUpdateDialog(
        title: 'Critical Update Required',
        message: 'A mandatory update (v$latestVersion) is available. Please update the app to continue.',
      ...
      try {
        ...
        final String? ignoredVersion = prefs.getString('ignored_update_version');
        if (ignoredVersion == latestVersion) {
          return; // Skip if user clicked Later / ignored this version
        }
      } catch (_) {}

      _showUpdateDialog(
        title: 'Update Available',
        message: 'A new version (v$latestVersion) of Kosmico Wellness is available with new features and improvements.',
        buttonText: 'Update Now',
        isMandatory: false,
        url: playstoreUrl,
        version: latestVersion,
