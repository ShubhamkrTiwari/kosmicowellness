...
class _ScanMealModuleState extends State<ScanMealModule> with SingleTickerProviderStateMixin {
  ...
  String _scanSource = 'OpenAI GPT-4o Vision';
  ...
  Future<void> _startScanWithImage(XFile image) async {
    setState(() {
      ...
      _scanStatusText = 'ANALYZING MEAL WITH OPENAI GPT-4O VISION...';
    });

    Map<String, dynamic>? foodResult;
    List<Map<String, dynamic>> alternates = [];
    String detectedSource = 'OpenAI Vision';
    ...
    try {
      ...
      if (aiResponse.data != null && !aiResponse.isFallback) {
        ...
        detectedSource = 'OpenAI GPT-4o Vision';
        ...
        if (foodResult!['isFood'] == false) {
          if (mounted) {
            setState(() {
              ...
              _alternateMatches = CareManager().foodDatabase.take(6).toList();
            });
          }
          return;
        }
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(aiResponse.errorMessage ?? 'Could not recognize meal via Cloud AI. Pick your dish below:'),
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('AI Scan Error: $e. Please pick your dish from below.'),
    ...
    try {
      ...
      if (aiResponse.data != null && !aiResponse.isFallback) {
        ...
        setState(() {
          ...
          _scanSource = 'OpenAI GPT-4o Vision + Refined Context';
        });
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                    ...
                    onPressed: () async {
                      ...
                      if (aiRes.data != null && mounted) {
                        setState(() {
                          _currentFoodData = aiRes.data;
                          _scanSource = 'OpenAI GPT-4o Search';
                          _isFoodDetected = true;
                          _showResult = true;
                        });
  ...
  Widget _buildNoFoodDetectedCard(ColorScheme colorScheme) {
    // Quick-pick popular Indian dishes and thalis
    final quickPicks = CareManager().foodDatabase.where((item) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'The cloud AI scanner is offline or could not match this photo. Tap any dish below to calculate its diabetic impact:',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 12, color: Colors.grey[700], height: 1.35),
          ),
          const SizedBox(height: 16),

          // Clinical Quick Pick dishes
          Row(
            children: [
              Icon(Icons.restaurant_menu_rounded, size: 16, color: colorScheme.primary),
              const SizedBox(width: 6),
              const Text(
                'Top Plate & Thali Choices (Tap to select):',
            ...
            itemBuilder: (context, index) {
              ...
              return InkWell(
                onTap: () {
                  setState(() {
                    _currentFoodData = item;
                    _scanSource = 'Selected Clinical Meal';
                    _isFoodDetected = true;
                    _showResult = true;
                  });
                  _loadReferencePreferencesForCurrentFood(item['name'] ?? '');
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(Icons.restaurant, color: riskColor, size: 18),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Column(
