...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  bool _isCOD = false;
  bool _isCodAdvancePayment = false;
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    debugPrint('Payment Success: ${response.paymentId}, OrderId: ${response.orderId}');
    _finalizeOrder(response);
  ...
  Future<void> _fetchDeliveryEstimation() async {
    ...
    try {
      ...
      final double totalWeight = 0.5 * CartManager().itemCount;
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      if (_isCOD) {
        final double advanceAmount = _deliveryFee + _gstAmount;
        if (advanceAmount <= 0) {
          setState(() => _isCodAdvancePayment = false);
          ...
          setState(() => _isCodAdvancePayment = true);
          debugPrint('Checkout: Calling /api/payment/cod-upfront/create with amount: $_finalTotal, upfront: $advanceAmount');
          final result = await ApiService.createCodUpfrontOrder(
            amount: _finalTotal,
          ...
          if (result['success'] && result['data'] != null) {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(result['message'] ?? 'Failed to initiate advance payment'), backgroundColor: Colors.red),
              );
            }
          }
        }
  ...
  Future<void> _finalizeOrder(PaymentSuccessResponse response) async {
    final token = UserManager().token;
    if (token != null) {
      if (_isCodAdvancePayment) {
        debugPrint('Checkout: Verifying COD upfront payment...');
        final result = await ApiService.verifyCodUpfrontPayment(
  ...
  Future<void> _syncOrderToShiprocket(String orderId, String? paymentId) async {
    try {
      ...
      final Map<String, dynamic> shiprocketData = {
        ...
        'breadth': 10,
        'height': 10,
        'weight': (0.5 * cart.itemCount) < 0.5 ? 0.5 : (0.5 * cart.itemCount),
      };
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildOnlineModeCard(!_isCOD, colorScheme),
                const SizedBox(height: 16),
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: _isPlacingOrder 
              ...
              : Text(
                  _isCOD && (_deliveryFee + _gstAmount) > 0
                    ? 'Pay ₹${(_deliveryFee + _gstAmount).toStringAsFixed(0)} Online & Place Order'
          ...
          if (_isCOD) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: const Color(0xFF8B5A2B).withValues(alpha: 0.3)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Pay Online Now (Delivery + GST)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF8B5A2B))),
                      Text('₹${(_deliveryFee + _gstAmount).toStringAsFixed(0)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF8B5A2B))),
                    ],
                  ),
                  const SizedBox(height: 4),
                  const Text('• Non-Refundable advance payment', style: TextStyle(fontSize: 11, color: Colors.grey)),
                  const SizedBox(height: 10),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Pay on Delivery (Product Price)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                      Text('₹${(cartManager.totalPrice - _discountAmount).toStringAsFixed(0)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildOnlineModeCard(bool isSelected, ColorScheme colorScheme) {
    return GestureDetector(
      onTap: () {
        if (_isCOD) {
          setState(() => _isCOD = false);
          _fetchDeliveryEstimation();
        }
      },
      child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      color: isSelected ? Colors.white.withValues(alpha: 0.8) : colorScheme.onSurfaceVariant,
                      fontSize: 12,
                    ),
                  ),
                ],
  ...
  Widget _buildCodCard(ColorScheme colorScheme) {
    final double advanceAmount = _deliveryFee + _gstAmount;
    final bool isSelected = _isCOD;

    return GestureDetector(
      onTap: () {
        if (!_isCOD) {
          setState(() => _isCOD = true);
          _fetchDeliveryEstimation();
        }
      },
      child: Stack(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: const Color(0xFF8B5A2B), // Warm brown color
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
          ...
          // Overlapping Green Badge on Top Right
          Positioned(
            top: -12,
            right: 16,
            child: Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xFF00B074), // Vibrant green matching screenshot
              ...
              child: Text(
                'Pay ₹${advanceAmount.toStringAsFixed(0)} now. Rest on delivery',
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 11,
                  fontWeight: FontWeight.bold,
